...
local autoTackleConnection = nil
...
toggleButton.Size = UDim2.new(0, 270, 0, 60)
...
speedButton.Size = UDim2.new(0, 270, 0, 60)
speedButton.Position = UDim2.new(0, 15, 0, 120)
...
speedBox.Size = UDim2.new(0, 270, 0, 40)
...
autoClickButton.Size = UDim2.new(0, 270, 0, 60)
autoClickButton.Position = UDim2.new(0, 15, 0, 230)
...
-- Botão para ativar/desativar auto gol
local autoGoalButton = Instance.new("TextButton")
autoGoalButton.Size = UDim2.new(0, 270, 0, 60)
autoGoalButton.Position = UDim2.new(0, 15, 0, 290)
autoGoalButton.BackgroundColor3 = Color3.new(0.4, 0.4, 0.4)
autoGoalButton.Text = "Auto Gol: OFF"
autoGoalButton.TextColor3 = Color3.new(1, 1, 1)
autoGoalButton.TextScaled = true
autoGoalButton.ZIndex = 101
autoGoalButton.Parent = frame
print("Botão Auto Gol criado")

-- Botão para teleportar para a bola
local teleportBallButton = Instance.new("TextButton")
teleportBallButton.Size = UDim2.new(0, 270, 0, 60)
teleportBallButton.Position = UDim2.new(0, 15, 0, 350)
...
-- Frame circular para estado minimizado
...
local function toggleMinimize()
    ...
    if isMinimized then
        ...
        if autoGoalEnabled then toggleAutoGoal() end
...
-- Função para verificar se outro jogador está com a bola
local function isBallWithOtherPlayer()
    local ball = findBall()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if distance < 5 then
                print("Bola com jogador: " .. player.Name)
                return true, player
            end
        end
    end
...
-- Função para simular rasteira (tackle)
local function simulateTackle(targetPlayer)
    pcall(function()
        local tackleEvent
        for _, obj in pairs(ReplicatedStorage:GetDescendants()) do
            if obj:IsA("RemoteEvent") and (obj.Name:lower():find("tackle") or obj.Name:lower():find("slide")) then
                tackleEvent = obj
                break
            end
        end
        if tackleEvent then
            tackleEvent:FireServer(targetPlayer.Character.HumanoidRootPart.Position)
            print("Rasteira aplicada em " .. targetPlayer.Name)
        else
            print("Erro: Evento de tackle não encontrado em ReplicatedStorage")
        end
    end)
end

-- Função para ativar/desativar auto gol
local function toggleAutoGoal()
    autoGoalEnabled = not autoGoalEnabled
    autoGoalButton.Text = autoGoalEnabled and "Auto Gol: ON" or "Auto Gol: OFF"
    if autoGoalEnabled then
        if isPlayerWithBall() and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            pcall(function()
            ...
            print("Erro: Jogador não está com a bola ou personagem não encontrado")
        end
        if autoTackleConnection then
            autoTackleConnection:Disconnect()
        end
        autoTackleConnection = RunService.Heartbeat:Connect(function()
            local hasBall, targetPlayer = isBallWithOtherPlayer()
            if hasBall and targetPlayer then
                simulateTackle(targetPlayer)
            end
            task.wait(0.3)
        end)
        print("Auto gol ativado")
    else
        if autoTackleConnection then
            autoTackleConnection:Disconnect()
            autoTackleConnection = nil
        end
        print("Auto gol desativado")
